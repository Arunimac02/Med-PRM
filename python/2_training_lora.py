...
# RNG state pickled inside Trainer checkpoints' rng_state.pth (used on
# resume_from_checkpoint). All checkpoints this script loads are self-produced
# or official meta-llama weights, so it's safe to restore the pre-2.6 default.
...
def parse_args():
    ...
    parser.add_argument("--risk_param", type=float, default=5.0, help="Risk parameter (μ), logged for reference.")
    parser.add_argument("--resume_from_checkpoint", type=str, default=None,
                        help="Path to a checkpoint dir to resume training from (optimizer/scheduler/step state included).")
    ...
    # LoRA (parameter-efficient fine-tuning, added to fit an 8B full fine-tune
    # into a single 40GB A100 without DeepSpeed/ZeRO)
    parser.add_argument("--use_lora", type=str, default="yes",
                        choices=["yes", "no"],
                        help="Train with a LoRA adapter instead of full fine-tuning.")
    ...
    # Online related
    # NOTE: type=str (not bool) on purpose -- argparse's bool("False") is True
    # for any non-empty string, which would silently force online mode on.
    parser.add_argument("--online", type=str, default="False", help="Enable online mode (True/False)")
...
def main():
    ...
    # 2b) LoRA Wrapping -------------------------------------------------
    # Base model stays frozen in bf16; only adapters (+ the embedding table,
    # since it now contains the freshly-added " ки" token) are trained.
    # This is what makes an 8B fine-tune fit on a single 40GB A100.
    use_lora = args.use_lora.lower() == "yes"
    if use_lora:
    ...
    # 7) Saving -------------------------------------------------------
    # If trained with LoRA, merge the adapter into the base weights so the
    # saved checkpoint is a normal full model (4_scoring_PRM.py loads it via
    # plain AutoModelForCausalLM.from_pretrained, with no peft awareness).
    if use_lora and isinstance(trainer.model, PeftModel):
