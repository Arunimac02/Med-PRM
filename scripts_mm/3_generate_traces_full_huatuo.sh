#!/bin/bash
# Full-scale multimodal trace generation, HuatuoGPT-Vision-7B-Qwen2.5VL
# only, across all 11,286 questions (vqa_rad + slake + medxpertqa_mm). Runs
# as its own SLURM job on its own GPU, in parallel with the Qwen run
# (3_generate_traces_full_qwen.sh) instead of both models sharing one GPU
...
# ~1280-tokens-per-image budget (1280*784, since Huatuo/Qwen2.5-VL uses
# 784 pixels/token -- 14px patch x 2x merge). Without this, a single
# high-resolution image could cost 11000+ tokens on its own (this is what
# actually crashed the first full-run attempt, job 19658750).
...
MODEL_PATH="FreedomIntelligence/HuatuoGPT-Vision-7B-Qwen2.5VL"
